...
function getSongKDetails($sid,$mode){

global $_Master_song_script, $_Master_albumsong_script, $_Master_upload_kars, $_Master_upload_akars;
    $table = 'SONGS';
  ...
  while ($i < $num_qry){
    ...
    if ($mode == 'ALBUMS'){
	echo "<form method='POST' enctype='multipart/form-data' action=$_Master_upload_akars><br>";
    }
    else {
	echo "<form method='POST' enctype='multipart/form-data' action=$_Master_upload_kars><br>";
    }
    echo "<input type=hidden name=sid value=$sid>\n";
    $msgFile = "Writeups/submitKaraokeClips_msg";
    if ($_GET['lang'] != 'E'){
    $msgFile .= '_malayalam';
    }
