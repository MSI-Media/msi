...
$cLink = msi_dbconnect();
printXHeader('');
getSongVDetails($sid,$mode);
printFancyFooters();
mysql_close($cLink);



function getSongVDetails($sid,$mode){
	 global $_Master_upload_video ,$_Master_upload_avideo ,$_Master_song_script  , $_Master_albumsong_script  ;
    $table = 'SONGS';
	...
	if ($_GET['lang'] == 'E'){
        ...
        echo "<tr><td colspan=2 class=psubheading align=center>$vidtag</td></tr>\n";
	...
	if ($mode == 'ALBUMS'){
	    echo "<form method='POST' enctype='multipart/form-data' action=$_Master_upload_avideo><br>";
	}
	else {
	    echo "<form method='POST' enctype='multipart/form-data' action=$_Master_upload_video><br>";
	}
	echo "<input type=hidden name=sid value=$sid>\n";
	$msgFile = "Writeups/submitVids_msg";
	if ($_GET['lang'] != 'E'){
	    $msgFile .= '_malayalam';
	}
