...
if ($securimage->check($_POST['captcha_code']) == false) {
  ...
  echo "<script>history.back();</script>";
    ...
    if ($pos != false){
      $fil = "php/temp/mal_lyrics/${sid}.html";
      if (file_exists("$fil")){
	$fil = "php/temp/mal_lyrics/${sid}.1.html";
	if (file_exists("$fil")){
	  $fil = "php/temp/mal_lyrics/${sid}.2.html";
	}
      }
      $lyricsismal = 1;
    }
    else {
      $fil = "php/temp/lyrics/${sid}.html";
      if (file_exists("$fil")){
	$fil = "php/temp/lyrics/${sid}.1.html";
	if (file_exists("$fil")){
	  $fil = "php/temp/lyrics/${sid}.2.html";
	}
      }
    }
//------------------------------------------------------
...
//------------------------------------------------------
  ...
  $name = "$name" . "@msiguest.com";
    ...
    printFancyFooters();
    mysql_close($cLink);
    mail("msiadmins@googlegroups.com","UPDATE LYRICS: MSI 2012 [msidb.org] Lyrics Addition","<br>($sid)<pre>$lyrics</pre><div style=\"background-color:#ffffcc\">First Verify whether these lyrics have been updated by another admin already: <a href=\"$_RootOfMedia/s.php?$sid\">Click Here To Verify the generated Lyrics File</a> <br>if not, <a href=\"$_RootOfMedia/admin/index.php?display=Lyrics\">Manage These Lyrics in Admin Panel</a><br><a href=\"$_RootOfMedia/admin/index.php?display=Update&sid=$sid&type=auto\">Manage This Song Details</a><br></div>",$mailHead);



    echo "<script>alert(\"Thanks for submitting comments on lyrics.\\nWe will make the necessary updates soon\");</script>\n";
    echo "<script>location=\"$_RootOfMedia/manageLyrics.php?song_id=${sid}\";</script>";
}
