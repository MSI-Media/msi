<?php session_start();
{
    ...
    $n = array();
    $n = buildArrayFromQuery("SELECT name from UNICODE_MAPPING WHERE unicode like \"$search%\" limit 1",'name');
    $new_match_string = '';
    $min_size = 1000;
    foreach ($n as $nelem){
   ...
   $db = $_POST['db'];
    if ($db == 'movies'){
       ...
       location_replace("$_Master_profile_script?artist=$search&category=year");
   }
   else {
